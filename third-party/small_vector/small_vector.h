...
#if defined(GCH_HAS_CPP20_CONSTEXPR) &&       \
    defined(GCH_LIB_IS_CONSTANT_EVALUATED) && \
...
concept NoThrowConvertibleTo =
    ...
    requires(
        typename std::add_rvalue_reference<From>::type (&f)(void) noexcept) {
...
struct default_buffer_size {
 ...
 public:
  ...
  // static_assert (sizeof (value_type) <= buffer_max, "`sizeof (T)` too
  // large");
...
template <typename T>
struct is_nothrow_swappable<T, decltype(swap(std::declval<T&>(),
                                             std::declval<T&>()))>
...
class inline_storage {
 public:
  ...
  static constexpr std::size_t element_size(void) noexcept {
    return sizeof(value_ty);
  }

  static constexpr std::size_t alignment(void) noexcept {
    return alignof(value_ty);
  }

  static constexpr unsigned num_elements(void) noexcept {
 ...
 private:
  typename std::aligned_storage<element_size(), alignment()>::type
      m_data[num_elements()];
};
...
#endif  // If you are using this with C++11 just don't use an allocator marked
        // as final :P
          >
...
class GCH_EMPTY_BASE allocator_inliner<Allocator, true> : private Allocator {
 ...
 public:
  ...
  GCH_CPP20_CONSTEXPR
  allocator_inliner& operator=(allocator_inliner&& other) noexcept(noexcept(
...
class allocator_inliner<Allocator, false> {
  ...
  template <bool IsNoOp = move_assign_is_noop,
            typename std::enable_if<!IsNoOp, bool>::type = false>
  GCH_CPP20_CONSTEXPR void maybe_assign(allocator_inliner&& other) noexcept(
      noexcept(std::declval<decltype(other.m_alloc)&>() =
                   std::move(other.m_alloc))) {
 ...
 public:
  ...
  GCH_CPP20_CONSTEXPR explicit allocator_inliner(
...
class GCH_EMPTY_BASE allocator_interface : public allocator_inliner<Allocator> {
 public:
  ...
  using difference_type = typename std::conditional<
      (static_cast<std::size_t>(
       ...
       static_cast<std::size_t>(
           (std::numeric_limits<typename std::allocator_traits<
      ...
      typename std::conditional<
          ...
          typename std::conditional<
              ...
              typename std::conditional<
                  ...
                  std::uint_fast64_t, size_type>::type>::type>::type>::type;
      ...
      typename std::conditional<
          ...
          typename std::conditional<
              ...
              typename std::conditional<
                  ...
                  std::int_fast64_t, difference_type>::type>::type>::type>::
      type;
 ...
 private:
  ...
  struct has_alloc_construct_impl<
      typename std::enable_if<is_complete<V>::value>::type, A, V, Args...>
 ...
 public:
  ...
  struct is_memcpyable_integral<
      From, To, typename std::enable_if<is_complete<From>::value>::type> {
...
#ifdef GCH_STDLIB_INTEROP
            || std::is_same<InputIt,
                            typename std::array<value_ty>::iterator>::value ||
            std::is_same<InputIt, typename std::array<
                                      value_ty>::const_iterator>::value ||
            (!std::is_same<value_ty, bool> &&
             (std::is_same<InputIt,
                           typename std::vector<value_ty>::iterator>::value ||
              std::is_same<InputIt, typename std::vector<
                                        value_ty>::const_iterator>::value)) ||
            std::is_same<InputIt,
                         decltype(std::begin(std::declval<std::valarray<
                                                 value_ty>&>()))>::value ||
            std::is_same<InputIt,
                         decltype(std::begin(std::declval<const std::valarray<
                                                 value_ty>&>()))>::value
#endif
            > {
  };
  ...
  struct is_memcpyable_iterator
      : bool_constant<
            is_memcpyable<decltype(*std::declval<InputIt>())>::value &&
  ...
  struct is_uninitialized_memcpyable_iterator
      : bool_constant<is_uninitialized_memcpyable<
                          V, decltype(*std::declval<InputIt>())>::value &&
                      is_contiguous_iterator<InputIt>::value> {};
...
#else
    std::fprintf(stderr,
                 "[gch::small_vector] The specified range is too long.");
    std::abort();
#endif
  }
  ...
  template <typename ForwardIt,
            typename ItDiffT =
                typename std::iterator_traits<ForwardIt>::difference_type,
            typename std::enable_if<!(numeric_max<size_ty>() <
                                      numeric_max<ItDiffT>()),
                                    bool>::type = false>
  GCH_NODISCARD static GCH_CPP17_CONSTEXPR size_ty
  ...
  GCH_NODISCARD static GCH_CPP17_CONSTEXPR Iterator
  ...
  template <typename Iterator,
            typename IteratorDiffT =
                typename std::iterator_traits<Iterator>::difference_type,
            typename Integer = IteratorDiffT>
  static GCH_CPP17_CONSTEXPR void unchecked_advance(Iterator& pos,
                                                    Integer n) noexcept {
  ...
  GCH_NODISCARD GCH_CPP20_CONSTEXPR ptr allocate_with_hint(size_ty n,
                                                           cptr hint) {
    return alloc_traits::allocate(allocator_ref(), static_cast<size_type>(n),
                                  hint);
  }
  ...
  template <typename ForwardIt,
            typename std::enable_if<
                is_uninitialized_memcpyable_iterator<ForwardIt>::value,
                bool>::type = true>
  ...
  uninitialized_copy(std::move_iterator<ForwardIt> first,
                     std::move_iterator<ForwardIt> last, ptr dest) noexcept {
    return uninitialized_copy(first.base(), last.base(), dest);
  }

  template <typename InputIt,
            typename std::enable_if<
                !is_uninitialized_memcpyable_iterator<InputIt>::value,
                bool>::type = false>
  GCH_CPP20_CONSTEXPR ptr uninitialized_copy(InputIt first, InputIt last,
                                             ptr d_first) {
    return default_uninitialized_copy(first, last, d_first);
  }

  template <typename InputIt>
  GCH_CPP20_CONSTEXPR ptr default_uninitialized_copy(InputIt first,
                                                     InputIt last,
                                                     ptr d_first) {
    ptr d_last = d_first;
    GCH_TRY {
      // Note: Not != because `using namespace std::rel_ops` can break
      // constexpr.
  ...
  template <typename A = alloc_ty, typename V = value_ty,
            typename std::enable_if<
                is_trivially_constructible<V>::value &&
 ...
 private:
  ...
  static GCH_CPP20_CONSTEXPR auto construct_at(
      value_ty* p,
...
class small_vector_base : public allocator_interface<Allocator> {
 ...
 protected:
  ...
  static_assert(
      alloc_interface::template is_complete<value_ty>::value ||
          InlineCapacity == 0,
  ...
  struct is_emplace_constructible_impl<
    ...
    using nothrow = bool_constant<noexcept(std::declval<AI&>().construct(
        std::declval<V*>(), std::declval<Args>()...))>;
  };

  template <typename... Args>
  struct is_emplace_constructible
      : is_emplace_constructible_impl<void, alloc_interface, value_ty,
                                      Args...> {};
  ...
  struct is_explicitly_nothrow_copy_insertable
      : std::integral_constant<
            bool, is_nothrow_emplace_constructible<V&>::value &&
  ...
  struct is_eraseable<AI, void_t<decltype(std::declval<AI&>().destroy(
                              std::declval<value_ty*>()))>> : std::true_type {};
...
#else
    std::fprintf(stderr,
                 "[gch::small_vector] The requested index was out of range.\n");
    std::abort();
#endif
  }
...
#else
    std::fprintf(stderr,
                 "[gch::small_vector] The requested increment was outside of "
                 "the allowed range.\n");
    std::abort();
#endif
  }
...
#else
    std::fprintf(stderr,
                 "[gch::small_vector] The required allocation exceeds the "
                 "maximum size.\n");
    std::abort();
#endif
  }
 ...
 private:
  class stack_temporary {
   ...
   private:
    ...
    typename std::aligned_storage<sizeof(value_ty), alignof(value_ty)>::type
        m_data;
  };
 ...
 protected:
  ...
  GCH_NODISCARD GCH_CPP14_CONSTEXPR size_ty checked_calculate_new_capacity(
  ...
  template <
      unsigned I, typename AT = alloc_traits,
  ...
  template <unsigned GreaterI, typename std::enable_if<(
  ...
  template <unsigned I, typename A = alloc_ty,
            typename std::enable_if<allocations_are_movable<A>::value>::type* =
                nullptr>
 ...
 public:
  ...
  GCH_CPP20_CONSTEXPR small_vector_base(
      bypass_tag, const small_vector_base<Allocator, I>& other,
 ...
 protected:
  ...
  template <typename InputIt,
            typename std::enable_if<std::is_assignable<
                value_ty&, decltype(*std::declval<InputIt>())>::value>::type* =
                nullptr>
  ...
  GCH_CPP20_CONSTEXPR ptr
  uninitialized_move(ptr first, ptr last, ptr d_first) noexcept(
      alloc_interface::template is_uninitialized_memcpyable_iterator<
          ptr>::value) {
  ...
  template <typename MovePolicy, typename InputIt,
            typename std::enable_if<
                std::is_same<MovePolicy, strong_exception_policy>::value,
                bool>::type = true>
  ...
  template <typename ForwardIt>
  GCH_CPP20_CONSTEXPR ptr insert_range_helper(ptr pos, ForwardIt first,
                                              ForwardIt last) {
  ...
  template <typename InputIt>
  static GCH_CPP20_CONSTEXPR auto unmove_iterator(
      std::move_iterator<InputIt> it) -> decltype(unmove_iterator(it.base())) {
    return unmove_iterator(it.base());
  }

  template <typename InputIt>
  static GCH_CPP20_CONSTEXPR auto unmove_iterator(
      std::reverse_iterator<InputIt> it)
  ...
  template <typename InputIt, typename std::enable_if<is_memcpyable_iterator<
                                  InputIt>::value>::type* = nullptr>
  ...
  GCH_CPP20_CONSTEXPR RandomIt copy_n_return_in(RandomIt first, size_ty count,
                                                ptr dest) {
#if defined(GCH_LIB_IS_CONSTANT_EVALUATED) && defined(__GLIBCXX__)
    if (std::is_constant_evaluated() &&
...
class small_vector
 ...
 public:
...
#endif
  GCH_CPP20_CONSTEXPR small_vector(
      size_type count, Generator g,
      const allocator_type& alloc = allocator_type())
      : base(count, g, alloc) {
  }
...
#endif
  GCH_CPP20_CONSTEXPR void
  assign(small_vector<T, I, Allocator>&& other) noexcept(
      I <= InlineCapacity &&
      (std::is_same<std::allocator<value_type>, Allocator>::value ||
       std::allocator_traits<
           Allocator>::propagate_on_container_move_assignment::value
            ...
            typename std::enable_if<
                std::is_base_of<std::input_iterator_tag,
                                typename std::iterator_traits<InputIt>::
                                    iterator_category>::value>::type* = nullptr>
#endif
  GCH_CPP20_CONSTEXPR iterator insert(const_iterator pos, InputIt first,
                                      InputIt last) {
  ...
  template <unsigned I>
  GCH_CPP20_CONSTEXPR small_vector& append(
      const small_vector<T, I, Allocator>& other)
#ifdef GCH_LIB_CONCEPTS
    requires CopyInsertable
#endif
  ...
  template <unsigned I>
  GCH_CPP20_CONSTEXPR small_vector& append(
      small_vector<T, I, Allocator>&& other)
#ifdef GCH_LIB_CONCEPTS
    requires MoveInsertable
#endif
  {
...
constexpr typename std::common_type<
    std::ptrdiff_t, typename std::make_signed<typename small_vector<
...
template <typename InputIt,
          unsigned InlineCapacity = default_buffer_size<std::allocator<
