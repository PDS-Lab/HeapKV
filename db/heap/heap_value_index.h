...
class HeapValueIndex {
 public:
  static constexpr size_t IndexSize = 24;
 ...
 public:
  ...
  HeapValueIndex(SequenceNumber seq, ext_id_t extent_number,
                 uint16_t block_offset, uint16_t block_cnt, uint32_t value_size,
  ...
  Status DecodeFrom(const Slice& slice) {
    ...
    packed_seqnum_compression_type_ = DecodeFixed64(slice.data() + 16);
    return Status::OK();
  }
  ...
  void EncodeTo(char* dst) const {
    ...
    EncodeFixed64(dst + 16, packed_seqnum_compression_type_);
  }
